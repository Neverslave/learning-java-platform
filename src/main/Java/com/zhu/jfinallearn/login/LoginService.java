...
public class LoginService {
    ...
    public Ret login(String userName ,String password , boolean keepLogin ,String loginIp){
        userName = userName.toLowerCase().trim();//不区分大小写忽略两端空格
        ...
        String hashPass = HashKit.sha256(salt+password);
        //未通过密码验证
        if(hashPass.equals( loginAccount.getPassword()) == false){
            return  Ret.fail("msg","用户名或密码不正确");
        }
        //如果用户勾选保持登录，暂定过期日期为三年，否则为120分钟，单位为秒
        long liveseconds = keepLogin ? 3*365*24*60*60 : 120*60;
        //传递给控制层cookie
        int maxAgeInSeconds = (int)(keepLogin ? liveseconds : -1);
        // expireAt 用于设置session 的过期时间点，需要转换为毫秒
        ...
        createLoginLog(loginAccount.getId(),loginIp); //创建登陆日志
